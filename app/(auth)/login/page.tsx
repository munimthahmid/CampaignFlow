...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    if (error) {
      setErrorMsg('Google sign-in is not enabled. Enable the Google provider in Supabase → Authentication → Providers → Google and add Client ID/Secret, then retry.')
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-fade-in">
        ...
        <div className="text-center space-y-3 mb-8">
          ...
          <p className="text-muted-foreground text-lg">Sign in to continue to CampaignFlow</p>
        </div>
          ...
          {GOOGLE_OAUTH_ENABLED && (
            <>
              ...
              >
                ...
                <span className="group-hover:gradient-text transition-all">Continue with Google</span>
              </button>
            </>
          )}
        </div>
        ...
        <div className="mt-8 space-y-4 text-center">
          <p className="text-sm text-muted-foreground">
            ...
            <Link href="/signup" className="gradient-text font-semibold hover:underline transition-all">
              Sign up for free
            </Link>
          </p>
